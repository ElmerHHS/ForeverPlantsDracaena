...
import telnetlib, socket, time, threading, math
...
class KawaBot:
    ...
    def telnet_process_options(self, socket, cmd, opt):
        ...
        if cmd == WILL and opt == ECHO:  # hex:ff fb 01 name:IAC WILL ECHO description:(I will echo)
            ...
            socket.sendall(
                IAC + WILL + TTYPE)  # hex(ff fb 18), name(IAC WILL TTYPE), descr(Dont worry, i'll send environment type)
        elif cmd == SB:
    ...
    def connect(self):
        ...
        time.sleep(0.5)  # Allow TELNET negotiation to finish
    ...
    def load_as_file(self, file_location=None):
        ...
        if file_location != None:
            ...
            text_split = [file_text[i:i + max_chars] for i in
                          range(0, len(file_text), max_chars)]  # Split AS code in sendable blocks
    ...
    def jmove_joint(self, joint_0, joint_1, joint_2, joint_3, joint_4, joint_5, speed=100, accuracy=10, acceleration=100,
              deceleration=100, break_cp=0):
    ...
    def lmove_cartesian(self, x, y, z, rx, ry, rz, speed=100, accuracy=10, acceleration=100,
              deceleration=100, break_cp=0):
        # EXAMPLE joint_trajectory_point = "\x02|\x01|45|11|00|10|400|100|100|0|-40|20|-50|0|10|100|\x03"
        msg_start = "\x02|\x01|"
        msg_seq_num = "|12|" + str(self.movement_seq_num).zfill(2)
        msg_end = msg_seq_num + "|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|\x03".format(
            speed, accuracy, acceleration, deceleration, break_cp,
            x, y, z, rx, ry, rz)
        joint_trajectory_point = msg_start + str(len(msg_end)) + msg_end
    ...
    def jmove_cartesian(self, x, y, z, rx, ry, rz, speed=100, accuracy=10, acceleration=100,
              deceleration=100, break_cp=0):
        # EXAMPLE joint_trajectory_point = "\x02|\x01|45|11|00|10|400|100|100|0|-40|20|-50|0|10|100|\x03"
        msg_start = "\x02|\x01|"
        msg_seq_num = "|13|" + str(self.movement_seq_num).zfill(2)
        msg_end = msg_seq_num + "|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|{}|\x03".format(
            speed, accuracy, acceleration, deceleration, break_cp,
            x, y, z, rx, ry, rz)
        joint_trajectory_point = msg_start + str(len(msg_end)) + msg_end
    ...
    def jmove_cartesian_check_pos(self, x, y, z, rx, ry, rz, speed=100):
        self.jmove_cartesian(x, y, z, rx, ry, rz, speed=speed)
        in_position = False
        timeout = time.time() + 10
        while not in_position:
            current_position = self.get_current_position()
            if math.isclose(current_position[0], x, abs_tol=0.05) \
                    and math.isclose(current_position[1], y, abs_tol=0.05) \
                    and math.isclose(current_position[2], z, abs_tol=0.05):
                in_position = True

            if time.time() > timeout:
                break

    def jmove_joint_check_pos(self, joint_0, joint_1, joint_2, joint_3, joint_4, joint_5, speed=100):
        self.jmove_joint(joint_0, joint_1, joint_2, joint_3, joint_4, joint_5, speed=speed)
        in_position = False
        timeout = time.time() + 10
        while not in_position:
            current_position = self.get_current_position(joint=True)
            if math.isclose(current_position[0], joint_0, abs_tol=0.02) \
                    and math.isclose(current_position[1], joint_1, abs_tol=0.02) \
                    and math.isclose(current_position[2], joint_2, abs_tol=0.02) \
                    and math.isclose(current_position[3], joint_3, abs_tol=0.02) \
                    and math.isclose(current_position[4], joint_4, abs_tol=0.02) \
                    and math.isclose(current_position[5], joint_5, abs_tol=0.02):
                in_position = True

            if time.time() > timeout:
                break

    def lmove_cartesian_check_pos(self, x, y, z, rx, ry, rz, speed=100):
        self.lmove_cartesian(x, y, z, rx, ry, rz, speed=speed)
        in_position = False
        timeout = time.time() + 10
        while not in_position:
            current_position = self.get_current_position()
    ...
    def get_current_position(self, joint=False):
        if joint:
            self.pose_server.send('j'.encode('utf-8'))
        else:
            self.pose_server.send('c'.encode('utf-8'))
        pose_byte_array = self.pose_server.recv(128)
        split_byte_array = pose_byte_array.split(b'|')

        def convert_bytes(byte_array):
            try:
                return float(str(byte_array, 'utf-8'))
            except:
                return 0

        positions = list(map(convert_bytes, split_byte_array[1:7]))
        return positions
